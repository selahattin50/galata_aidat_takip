...
const YearlyReportView: React.FC<YearlyReportViewProps> = ({ transactions, units, onClose, buildingName, onAddFile }) => {
  ...
  const handleCreateAndSharePdf = async (mode: 'share' | 'download') => {
    ...
    try {
      ...
      const fileName = `${sanitizedBuildingName}_${selectedYear}_Yili_Bilancosu.pdf`;
    ...
    <div className="fixed inset-0 z-[200] bg-[#030712] flex flex-col animate-in slide-in-from-bottom duration-500 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-32 no-scrollbar">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          ...
          <button onClick={() => setIsYearPickerOpen(true)} className="bg-[#1e293b] h-9 rounded-lg px-3 flex items-center justify-between border border-white/5 shadow-inner">
        ...
        <div className="bg-[#1e293b]/40 rounded-[32px] border border-white/5 px-2 py-6 mb-6 shadow-2xl relative overflow-hidden ring-1 ring-white/5">
          <div className="grid grid-cols-2 gap-3 relative">
            ...
            <div>
              ...
              <div className="mt-6 text-right"><span className="text-[16px] font-black text-white tracking-tighter">{formatCurrency(totalIncome - previousDevir)}</span></div>
            </div>
          </div>
        </div>
        ...
        <div id="pdf-report-content" ref={reportRef} style={{ width: '800px', backgroundColor: '#ffffff', color: '#000', padding: '40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px', backgroundColor: '#000', border: '2px solid #000' }}>
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px' }}>
              ...
              <div style={{ marginTop: 'auto', paddingTop: '20px', textAlign: 'right' }}><span style={{ fontSize: '28px', fontWeight: '900', color: '#22c55e' }}>{formatCurrency(totalIncome)}</span></div>
            </div>
          </div>
          <div style={{ marginTop: '30px', padding: '20px', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', display: 'flex', justifySelf: 'space-between', alignItems: 'center' }}>
            <div><p style={{ margin: '0', fontSize: '10px', fontWeight: '900', color: '#64748b' }}>YILLIK KASA DURUMU</p><p style={{ margin: '0', fontSize: '14px', fontWeight: '900' }}>{selectedYear} SONU</p></div>
