...
import { ChevronDown, ArrowLeft, FileText, Share2, Loader2, FileDown, Check, Wallet, Calendar, MessageCircle, Building, Inbox } from 'lucide-react';
...
const YearlyReportView: React.FC<YearlyReportViewProps> = ({ transactions, units, onClose, buildingName, onAddFile }) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pt-0 pb-32">
      {/* Top Controls matched to screenshot style */}
      <div className="sticky top-0 z-[100] -mx-4 px-4 pt-4 pb-4 bg-[#030712]/95 backdrop-blur-3xl border-b border-white/5 shadow-2xl">
        ...
        <div className="flex justify-end items-center space-x-2">
          ...
          <div className="relative">
            <button 
              onClick={() => { setIsYearPickerOpen(!isYearPickerOpen); setShowVaultPicker(false); }} 
              className="bg-[#1e293b] rounded-xl h-10 px-4 flex items-center space-x-3 border border-white/5 active:bg-white/10 transition-all shadow-lg"
            >
              <span className="text-[11px] font-black text-white uppercase tracking-widest">{selectedYear}</span>
              <Calendar size={14} className="text-zinc-400" />
            </button>
            {isYearPickerOpen && (
              <div className="absolute top-full right-0 z-[150] mt-1 w-28 bg-[#1e293b] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in">
                {years.map(y => (
                  <button key={y} onClick={() => { setSelectedYear(y); setIsYearPickerOpen(false); }} className={`w-full py-3 px-4 text-center text-[10px] font-black border-b border-white/5 last:border-0 hover:bg-white/5 ${selectedYear === y ? 'text-green-400 bg-white/5' : 'text-white/60'}`}>{y}</button>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="relative">
            <button 
              onClick={() => { setShowVaultPicker(!showVaultPicker); setIsYearPickerOpen(false); }} 
              className="bg-[#1e293b] rounded-xl h-10 px-4 flex items-center space-x-3 border border-white/5 active:bg-white/10 transition-all shadow-lg"
            >
              <span className="text-[11px] font-black text-white uppercase tracking-widest">{selectedVault === 'genel' ? 'GENEL' : 'DEMİRBAŞ'}</span>
              <Wallet size={14} className="text-zinc-400" />
            </button>
            {showVaultPicker && (
              <div className="absolute top-full right-0 z-[150] mt-1 w-32 bg-[#1e293b] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in">
                <button onClick={() => { setSelectedVault('genel'); setShowVaultPicker(false); }} className={`w-full py-3 px-4 text-left flex items-center justify-between border-b border-white/5 ${selectedVault === 'genel' ? 'text-green-400' : 'text-white/60'}`}>
                  <span className="text-[10px] font-black uppercase">GENEL</span>
      ...
      {/* Report Content matched to screenshot */}
      <div className="mt-6 px-1" ref={reportRef}>
        <div className="bg-white rounded-[32px] border border-gray-300 overflow-hidden shadow-2xl">
          ...
          <div className="grid grid-cols-2 min-h-[500px] relative">
            ...
            <div className="p-4 flex flex-col space-y-4">
              ...
              <div className="mt-auto pt-6 text-right border-t-2 border-gray-300">
                <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">TOPLAM</p>
                <span className="text-[18px] font-black text-green-600 tracking-tighter">{formatCurrency(totalIncome)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 space-y-3 px-2">
           <div className="flex justify-between items-center py-4 px-6 bg-gray-100 rounded-2xl border-2 border-gray-300 shadow-xl">
             <div className="flex flex-col">
                ...
                <span className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">{selectedYear} SONU</span>
             </div>
