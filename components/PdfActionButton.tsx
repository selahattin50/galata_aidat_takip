...
const PdfActionButton: React.FC<PdfActionButtonProps> = ({ type, onClick, disabled = false }) => {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="h-12 bg-[#1e293b] rounded-xl border border-white/10 flex items-center px-2.5 space-x-3 active:bg-white/10 active:scale-[0.98] transition-all shadow-[0_10px_20px_rgba(2,6,23,0.24)] disabled:opacity-50 disabled:active:scale-100"
    >
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center border shadow-inner ${isDownload ? 'bg-red-500/15 border-red-500/25' : 'bg-blue-600/25 border-blue-500/25'}`}>
        {isDownload ? (
